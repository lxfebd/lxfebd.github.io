...
const getMockReply = (message: string): string => {
  ...
  if (lower.includes("鱼") || lower.includes("喂")) {
    return "嗷呜！好吃喵！再给本喵来一条喵~";
  }
  if (lower.includes("你好") || lower.includes("hi") || lower.includes("hello")) {
    return "喵呜~ 你好呀铲屎官！今天也要努力敲代码喵！";
  }
  if (lower.includes("代码") || lower.includes("bug") || lower.includes("程序")) {
    return "本喵觉得你的代码写得还不错喵...才怪！快去修bug喵！";
  }
  if (lower.includes("方舟") || lower.includes("ark")) {
    return "罗德岛的干员们都在等你呢喵！快去刷关卡喵~";
  }
  return "喵~ 本喵正在思考人生的意义...其实是想吃小鱼干喵！";
};
...
export default function CyberCat() {
  ...
  const handleFeed = (e: React.MouseEvent) => {
    ...
    setTimeout(() => {
      const reply = getMockReply("我刚刚喂了你一条美味的小鱼干！你有什么表示？");
      speak(reply, 8000);
      setIsThinking(false);
    }, 1500);
  };
